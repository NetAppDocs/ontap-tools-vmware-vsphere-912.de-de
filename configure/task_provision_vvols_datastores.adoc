...
Sie können den VVols Datastore bereitstellen, wenn die VASA Provider-Funktion aktiviert ist. Siehe, link:../deploy/task_enable_vasa_provider_for_configuring_virtual_datastores.html["Aktivieren Sie VASA Provider zur Konfiguration von virtuellen Datastores"] Entsprechende Details. Der
VVols Datastore unterstützt NFS-, iSCSI-, FC/FEoE- und NVMe/FC-Protokolle.
