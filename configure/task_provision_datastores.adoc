...
Sie können entweder den Typ *NFS*, *VMFS* oder *VVols* auswählen, um einen Datastore zu konfigurieren.
Bei Auswahl von VVols wird das NVMe-/FC-Protokoll verfügbar.
...
Die Option *Datastore-Daten über die verteilen
ONTAP Cluster* zur Bereitstellung eines FlexGroup Volumes auf
Storage-System. Auswahl dieser Option
Deaktiviert automatisch das Kontrollkästchen *Speicher verwenden
Capability Profile für Provisioning*.
...
NOTE: Wenn VASA Provider aktiviert ist, dann können Sie
Storage-Funktion nutzen können
Profile.
